...
int numPaths(std::unordered_set<std::string> &vis, std::unordered_map<std::string, std::unordered_set<std::string>> &adj, std::string cur, int cnt, bool partB) {
    if (cur == "out") {
    ...
    for (auto s : adj[cur]) {
        if (!vis.contains(s)) {
            vis.insert(s);
        cnt = numPaths(vis, adj, s, cnt, partB);
            vis.erase(s);
        }
    }
    return cnt;
...
int partA(std::unordered_map<std::string, std::unordered_set<std::string>> &adj) {
    ...
    vis.insert("you");
    res = numPaths(vis, adj, "you", 0, false);
...
int partB(std::unordered_map<std::string, std::unordered_set<std::string>> &adj) {
    ...
    std::unordered_set<std::string> vis;
    vis.insert("svr");
    res = numPaths(vis, adj, "svr", 0, true);
